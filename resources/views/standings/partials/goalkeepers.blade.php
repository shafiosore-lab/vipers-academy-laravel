<div class="row g-4">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold text-primary mb-1">{{ $league }} League Goalkeeper Rankings</h2>
            ...
            <div class="d-flex gap-2">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="{{ route('standings.goalkeepers', ['season' => $season, 'league' => $league]) }}?view=all">All Keepers</a></li>
                        <li><a class="dropdown-item" href="{{ route('standings.goalkeepers', ['season' => $season, 'league' => $league]) }}?view=top">Top 10</a></li>
                    </ul>
                </div>
            </div>
        </div>
...
@if($goalkeeperRankings->count() > 0)
<div class="row">
    <div class="col-12">
        <div class="card standings-card">
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0 goalkeepers-table">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th class="text-center" width="80" title="Rating">Rating</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($goalkeeperRankings as $keeper)
                            <tr class="{{ $keeper->is_vipers_keeper ? 'table-primary' : '' }}">
                                ...
                                <td class="text-center fw-semibold text-success">{{ $keeper->clean_sheets ?? '-' }}</td>
                                ...
                                <td class="text-center">
                                    @if($keeper->rating)
                                        <span class="rating-badge {{ $keeper->rating >= 8 ? 'excellent' : ($keeper->rating >= 7 ? 'good' : 'average') }}">
                                            {{ number_format($keeper->rating, 1) }}
                                        </span>
                                    @else
                            ...
                            <tr>
                                <td colspan="8" class="text-center text-muted py-5">
                                    <i class="fas fa-hand-paper fa-4x mb-4"></i>
                                    <h4>No Goalkeeper Data Available</h4>
                                    <p>Goalkeeper rankings for {{ $season }} will be updated soon.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
...
.goalkeepers-table .position-sticky {
    position: sticky;
    background: white;
    z-index: 10;
}

.goalkeepers-table .keeper-cell {
    min-width: 200px;
}

        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0" width="100%">
                    ...
                    <tbody>
                        {{-- FIX 5: Move $approvalColors outside the loop --}}
                        @php
                            $approvalColors = [
                                'pending'  => 'warning',
                                'approved' => 'success',
                        ...
                        {{-- FIX 4: Compute display name once per row --}}
                        @php
                            $displayName = $team->team_name ?? $team->team?->name ?? 'Unknown Team';
                        @endphp
                        <tr>
                            <td class="py-2 align-middle">
                                <a href="{{ route('super-admin.tournaments.teams.players.index', [$tournament->id, $team->id]) }}"
                            ...
                            <td class="py-2 align-middle">
                                <a href="{{ route('super-admin.tournaments.teams.players.index', [$tournament->id, $team->id]) }}"
                                   class="badge bg-info text-decoration-none">
                                    {{ $team->squads->count() }} / {{ $tournament->squad_limit }}
                                </a>
                            </td>
                            <td class="py-2 align-middle">
                                @php $colorKey = $approvalColors[$team->approval_status] ?? 'secondary'; @endphp
                                <span class="badge bg-{{ $colorKey }}-subtle text-{{ $colorKey }}">
                                    {{ ucfirst($team->approval_status) }}
                                </span>
                            </td>
                            <td class="py-2 align-middle small">
                                {{-- FIX 3: Null-safe date formatting --}}
                                {{ $team->registration_date?->format('M d, Y') ?? '-' }}
                            </td>
                            <td class="py-2 align-middle">
                                @if($team->approval_status === 'pending')
                                    <form action="{{ route('super-admin.tournaments.teams.approve', [$tournament->id, $team->id]) }}"
                                    ...
                                    <button type="button"
                                            ...
                                            data-bs-target="#rejectModal{{ $team->id }}">
...
{{-- ============================================================
     FIX 2: Reject Modals moved OUTSIDE the table entirely
     ============================================================ --}}
@foreach($teams as $team)
@php $rejectDisplayName = $team->team_name ?? $team->team?->name ?? 'N/A'; @endphp
<div class="modal fade" id="rejectModal{{ $team->id }}" tabindex="-1" aria-labelledby="rejectModalLabel{{ $team->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('super-admin.tournaments.teams.reject', [$tournament->id, $team->id]) }}" method="POST">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title small" id="rejectModalLabel{{ $team->id }}">Reject Team Registration</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p class="small">
                        You are about to reject the registration for
                        <strong>{{ $rejectDisplayName }}</strong>.
                    </p>
                    <div class="mb-3">
...
{{-- ============================================================
     Bulk Upload Modal
     FIX 1: Renamed checkbox id to "bulk_auto_approve"
     ============================================================ --}}
<div class="modal fade" id="bulkUploadModal" tabindex="-1" aria-labelledby="bulkUploadModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('super-admin.tournaments.teams.bulk-upload', $tournament->id) }}"
                  method="POST"
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="teams_file" class="form-label fw-bold">
                    ...
                    <div class="mb-3">
                        {{-- FIX 1: Unique id "bulk_auto_approve" --}}
                        <label for="bulk_auto_approve" class="form-check">
                            <input type="checkbox"
                                   class="form-check-input"
                                   id="bulk_auto_approve"
                                   name="auto_approve"
                                   value="1">
...
{{-- ============================================================
     Add Team Modal
     FIX 1: Renamed checkbox id to "add_auto_approve"
     FIX 6: Replaced deprecated "font-weight-bold" with "fw-bold"
     ============================================================ --}}
<div class="modal fade" id="addTeamModal" tabindex="-1" aria-labelledby="addTeamModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('super-admin.tournaments.teams.store', $tournament->id) }}"
                  method="POST"
                  id="addTeamForm">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="team_id" name="team_id">
                            ...
                            @forelse($availableTeams as $availableTeam)
                                <option value="{{ $availableTeam->id }}">
                                    {{ $availableTeam->name }} ({{ $availableTeam->organization?->name ?? 'No Organization' }})
                                </option>
                            @empty
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                               ...
                               id="new_team_name"
                               name="team_name"
                               placeholder="Enter team name if not selecting above">
                    </div>
                    ...
                    <div class="card bg-light mb-3">
                        <div class="card-header py-2">
                            {{-- FIX 6: "font-weight-bold" → "fw-bold" --}}
                            <h6 class="m-0 small fw-bold text-primary">Team Location</h6>
                        ...
                        <div class="card-body py-2">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-2">
                                    <label for="country" class="form-label small fw-bold">
                    ...
                    <div class="mb-3 form-check">
                        {{-- FIX 1: Unique id "add_auto_approve" --}}
                        <input type="checkbox"
                               class="form-check-input"
                               id="add_auto_approve"
                               name="auto_approve"
                               value="1">
                        <label class="form-check-label" for="add_auto_approve">
